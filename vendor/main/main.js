...
{
    $.get( '../dlms-web-commons/menu/setMenuState.action?state=VISIBLE' );        
};
    
dhis2.leftBar.setMenuExtended = function()
{
    $.get( '../dlms-web-commons/menu/setMenuState.action?state=EXTENDED' );
};
    
dhis2.leftBar.setMenuHidden = function()
{        
    $.get( '../dlms-web-commons/menu/setMenuState.action?state=HIDDEN' );
};
    
dhis2.leftBar.openHelpForm = function( id )
{
    window.open( "../dlms-web-commons/help/viewDynamicHelp.action?id=" + id, "Help", "width=800,height=600,scrollbars=yes" );
};
